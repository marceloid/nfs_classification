...
if not pickle_file.exists():
    df_nfse = pd.read_csv(
        'poc3v2.csv', on_bad_lines='warn', sep='|', dtype=colunas_tipos, parse_dates=colunas_datas
    )
...
# Criando um regex com os nomes da lista usando word boundaries (\b) para evitar correspondências parciais
time_ini = time()
regex_nomes = r'\b(' + r'|'.join(nomes_comuns_br_freq_maior_que_100) + r')\b'


# Substituindo os nomes por "AAAA" no texto
